...
static int batch_scratch_init(BatchScratch *scratch,
                              const IroConv1d *conv, int rows) {
    memset(scratch, 0, sizeof(*scratch));
    if (conv->kernel != 7 || conv->in_channels != conv->out_channels) return -1;
    scratch->rows = rows;
    scratch->channels = conv->out_channels;
    scratch->bytes = sizeof(float) * 7u * (size_t)rows * conv->out_channels;
    void *aligned = NULL;
    if (posix_memalign(&aligned, 64, scratch->bytes) != 0) return -1;
    scratch->tap_outputs = aligned;
    return 0;
}

static void batch_scratch_free(BatchScratch *scratch) {
    free(scratch->tap_outputs);
    memset(scratch, 0, sizeof(*scratch));
}
...
static int run_batch(const IroConv1d *conv, const float *alpha,
                     const float *input, int frames, float *output,
                     int rows_cap, BatchScratch *scratch) {
    int halo = conv->dilation * (conv->kernel - 1);
    float *activated = malloc(sizeof(float) *
                              (size_t)(rows_cap + halo) * conv->in_channels);
    if (!activated) return -1;
    ...
    for (int start = 0; start < frames; start += rows_cap) {
        ...
        if (full_batch) {
            for (int tap = 0; tap < conv->kernel; tap++) {
                ...
                int output_end = frames - shift;
                if (output_end > block_end) output_end = block_end;
                if (output_start != start || output_end != block_end) {
                    full_batch = 0;
                    break;
                }
            }
        ...
        if (full_batch) {
            ...
            const CBLAS_TRANSPOSE transb[1] = {CblasTrans};
            const MKL_INT m[1] = {rows};
            const MKL_INT n[1] = {conv->out_channels};
            const MKL_INT k[1] = {conv->in_channels};
            const MKL_INT lda[1] = {conv->in_channels};
            const MKL_INT ldb[1] = {weight_stride};
            const MKL_INT ldc[1] = {conv->out_channels};
            const MKL_INT group_size[1] = {7};
            const float one[1] = {1.0f};
            const float zero[1] = {0.0f};
            const float *a[7];
            const float *b[7];
            float *c[7];
            size_t plane = (size_t)rows * conv->out_channels;
            for (int tap = 0; tap < 7; tap++) {
                int shift = tap * conv->dilation - conv->padding;
                int input_start = start + shift;
                a[tap] = activated +
                         (size_t)(input_start - input_min) * conv->in_channels;
                b[tap] = conv->weight + (size_t)tap * conv->in_channels;
                c[tap] = scratch->tap_outputs + (size_t)tap * plane;
            }
            cblas_sgemm_batch(CblasRowMajor, transa, transb,
                              m, n, k, one, a, lda, b, ldb,
                              zero, c, ldc, 1, group_size);

            memcpy(block_output, c[0], sizeof(float) * plane);
            for (int tap = 1; tap < 7; tap++) {
                const float *src = c[tap];
                for (size_t i = 0; i < plane; i++) block_output[i] += src[i];
            }
            continue;
        }
...
static int bench_layer(const IroDACVAEDecoder *decoder, const char *golden,
    ...
    if (!input || !base || !cand || !tb || !tc ||
        batch_scratch_init(&scratch, conv, rows) != 0)
        return -1;
    run_direct(conv, unit->alpha0, input, frames, base, rows);
    run_batch(conv, unit->alpha0, input, frames, cand, rows, &scratch);
    ...
    for (int p = 0; p < pairs; p++) {
        ...
        if (!(p & 1)) {
            ...
            t0 = now_seconds(); run_batch(conv, unit->alpha0, input, frames, cand, rows, &scratch); tc[p] = now_seconds()-t0;
        } else {
            t0 = now_seconds(); run_batch(conv, unit->alpha0, input, frames, cand, rows, &scratch); tc[p] = now_seconds()-t0;
            t0 = now_seconds(); run_direct(conv, unit->alpha0, input, frames, base, rows); tb[p] = now_seconds()-t0;
        }
        printf("{\"type\":\"pair\",\"stage\":%d,\"residual\":%d,\"channels\":%d,\"dilation\":%d,\"pair\":%d,\"baseline\":%.9f,\"batch\":%.9f,\"speedup\":%.6f}\n",
               stage, residual, conv->in_channels, conv->dilation, p+1,
               tb[p], tc[p], tb[p]/tc[p]);
        fflush(stdout);
    }
    double mb = median(tb, pairs), mc = median(tc, pairs);
    printf("{\"type\":\"summary\",\"stage\":%d,\"residual\":%d,\"frames\":%d,\"channels\":%d,\"dilation\":%d,\"rows\":%d,\"pairs\":%d,\"baseline_median\":%.9f,\"batch_median\":%.9f,\"speedup\":%.6f,\"max_abs\":%.9g,\"mae\":%.9g,\"scratch_bytes\":%zu}\n",
           stage, residual, frames, conv->in_channels, conv->dilation, rows, pairs,
           mb, mc, mb/mc, max_abs, mae, scratch.bytes);
    fflush(stdout);
