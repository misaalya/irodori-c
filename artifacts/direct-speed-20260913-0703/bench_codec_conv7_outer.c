...
static int direct_outer_threads = 1;
static int direct_only = 0;
...
static int run_direct_worker(const IroConv1d *conv, const float *alpha,
                             const float *input, int frames, float *output,
                             int rows_cap, int worker_index,
                             int worker_count) {
    ...
    for (int start = worker_index * rows_cap; start < frames;
         start += rows_cap * worker_count) {
...
static void *run_direct_thread(void *opaque) {
    DirectWorkerArgs *args = opaque;
#ifdef IRO_USE_TORCH_MKL
    int previous_threads = MKL_Set_Num_Threads_Local(1);
#endif
    args->status = run_direct_worker(
        args->conv, args->alpha, args->input, args->frames, args->output,
        args->rows_cap, args->worker_index, args->worker_count);
#ifdef IRO_USE_TORCH_MKL
...
static int run_direct(const IroConv1d *conv, const float *alpha,
                      const float *input, int frames, float *output,
                      int rows_cap) {
    if (direct_outer_threads == 1)
        return run_direct_worker(conv, alpha, input, frames, output, rows_cap,
                                 0, 1);

    pthread_t threads[2];
    DirectWorkerArgs args[2];
    int created = 0;
    for (int worker = 0; worker < 2; worker++) {
        args[worker] = (DirectWorkerArgs){
            .conv = conv,
            .alpha = alpha,
            .input = input,
            .frames = frames,
        ...
        if (pthread_create(&threads[worker], NULL, run_direct_thread,
                           &args[worker]) != 0)
            break;
        created++;
    }
    ...
    if (created != 2 || args[0].status != 0 || args[1].status != 0)
        return -1;
    return 0;
}
...
static int bench_layer(const IroDACVAEDecoder *decoder, const char *golden_dir,
    ...
    if (direct_only) {
        if (run_direct(conv, unit->alpha0, input, frames, direct, direct_rows) != 0) {
            free(input); free(im2col); free(direct);
            free(im2col_times); free(direct_times);
            return -1;
        }
        for (int pair = 0; pair < pairs; pair++) {
            ...
            if (run_direct(conv, unit->alpha0, input, frames, direct,
                           direct_rows) != 0) {
                free(input); free(im2col); free(direct);
                free(im2col_times); free(direct_times);
                return -1;
            ...
            printf("{\"type\":\"codec_conv7_direct\",\"backend\":\"%s\","
                   "\"outer_threads\":%d,\"stage\":%d,\"residual\":%d,"
                   "\"frames\":%d,\"channels\":%d,\"dilation\":%d,"
                   "\"direct_rows\":%d,\"pair\":%d,"
                   "\"direct_seconds\":%.9f,\"checksum\":%.17g}\n",
                   iro_ops_backend_name(), direct_outer_threads, stage, residual,
                   frames, conv->in_channels, conv->dilation, direct_rows,
                   pair + 1, direct_times[pair], checksum(direct, count));
        }
        free(input); free(im2col); free(direct);
        free(im2col_times); free(direct_times);
        return 0;
    ...
    for (int pair = 0; pair < pairs; pair++) {
        ...
        printf("{\"type\":\"codec_conv7_pair\",\"backend\":\"%s\","
               ...
               iro_ops_backend_name(), direct_outer_threads,
               stage, residual, frames,
...
int main(int argc, char **argv) {
    ...
    const char *direct_only_env = getenv("IRO_CODEC_DIRECT_ONLY");
    direct_only = direct_only_env && strcmp(direct_only_env, "0") != 0;
    if ((!direct_only && pairs < 3) || pairs < 1) return 2;
    int threads = 2;
    const char *threads_env = getenv("IRO_NUM_THREADS");
    if (threads_env && parse_positive_int(threads_env, &threads) != 0) return 2;
    const char *outer_env = getenv("IRO_CODEC_OUTER_THREADS");
    if (outer_env && parse_positive_int(outer_env, &direct_outer_threads) != 0)
        return 2;
    if (direct_outer_threads != 1 && direct_outer_threads != 2) return 2;
    iro_ops_set_threads(threads);
    fprintf(stderr,
            "bench_codec_conv7: backend=%s inner_threads=%d active=%d outer_threads=%d\n",
            iro_ops_backend_name(), threads, iro_ops_get_threads(),
            direct_outer_threads);
